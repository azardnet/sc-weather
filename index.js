...
const handleMouseMoveOnInfo = () => {
  ...
  els.lUpdate.innerHTML = `${translate[`${isPersianCharacter ? 'fa' : 'en'}`].lastUpdate} ${timeAgo(
    lastUpdate,
...
function computeUI(result, city, interval) {
  ...
  if (cityVideoData) {
    ...
    let currentVideoIndex = parseInt(localStorage.getItem(lastVideoIndexKey) || '0', 10)
    ...
    if (currentVideoIndex === 0) {
      ...
      videoSrc = require(`./static/videos/${result.id}-${currentVideoIndex + 1}.mp4`)
    }
  ...
  if (result && result.main) {
    ...
    els.wFeelsT.innerHTML = translate[isPersianCharacter ? 'fa' : 'en'].FeelsLike
