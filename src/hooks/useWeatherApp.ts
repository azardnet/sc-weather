...
export function useWeatherApp() {
  ...
  const [mapOpacity, setMapOpacity] = useState(
    () => +(localStorage.getItem("opacity") || DEFAULT_OPACITY),
  );
  ...
  const [clock24Hour, setClock24Hour] = useState(
    () => localStorage.getItem("clock_24h") === "true",
  );
  ...
  const [speed, setSpeed] = useState<SpeedSnapshot>({
    status: "error",
    text: "",
    trend: null,
  });
  ...
  useEffect(() => {
    clock24HourRef.current = clock24Hour;
  }, [clock24Hour]);
        ...
        try {
          window.ymaps.ready(() => {
            mapInstanceRef.current = new window.ymaps.Map(mapRef.current, {
              center:
                lat && lon ? [lat, lon] : [cacheDataRef.current.lat, cacheDataRef.current.lon],
              zoom: 13,
              controls: [],
            });
            markLoaded();
          ...
          const isPersian = checkPersianCharacters(
            cityFromStorage(localStorage.getItem("last_search") || DEFAULT_CITY),
          );
  ...
  const setupVideoBackground = useCallback(
    (cityId: number, cityVideoData: (typeof CITY_HAVE_VIDEO)[number]) => {
      setShowVideo(true);
      ...
      let index = Number.parseInt(localStorage.getItem(key) || "0", 10);
        ...
        if (result && city && !result.message) {
          ...
          if (result.coord?.lat) {
            ...
            if (hasVideo) {
              ...
              const randomIndex =
                randomIntFromInterval(0, imageCount !== undefined ? imageCount - 1 : Number.NaN) ||
                0;
              const imageId = Array.isArray(cityImageData?.id)
                ? cityImageData.id[0]
                : cityImageData?.id;
  ...
  const fetchMarketPrices = useCallback(async () => {
    ...
    try {
      ...
      if (!data?.stats?.["usdt-rls"] || !data.stats["btc-usdt"]) {
  ...
  const fetchGoldPrices = useCallback(async () => {
    const isFa = checkPersianCharacters(cityFromStorage(localStorage.getItem("last_search")));
    const setGoldError = () =>
      setPrices((p: PricesState) => ({ ...p, gold: isFa ? "خطا" : "error" }));
  ...
  useEffect(() => {
    applyTheme(color);
    setTimeout(() => InitiateSpeedDetection(setSpeed), 400);
  ...
  useEffect(() => {
    ...
    const speedTimer = setInterval(() => {
      MeasureConnectionSpeed(setSpeed);
    }, SPEED_DETECTION_DELAY);
    ...
    const clockTimer = setInterval(() => {
      ...
      const use24Hour = clock24HourRef.current;
      const midday = use24Hour ? "" : hours >= 12 ? "PM" : "AM";
      const hourValue = use24Hour ? updateTime(hours) : updateTime(hours % 12 || 12);
      const minStr = updateTime(now.getMinutes());
      const secStr = updateTime(now.getSeconds());

      if (clockSoundRef.current && now.getMinutes() === 0 && now.getSeconds() === 0) {
      ...
      setClock({
        hour: `${isPersian ? NumbersToPersian(hourValue) : hourValue}:${isPersian ? NumbersToPersian(minStr) : minStr}`,
  ...
  const onSettingsReset = () => {
    ...
    localStorage.setItem("clock_24h", "false");
  };
  ...
  return {
    ...
    settings: {
      ...
      onClock24HourChange: (checked: boolean) => {
        setClock24Hour(checked);
        localStorage.setItem("clock_24h", String(checked));
      },
